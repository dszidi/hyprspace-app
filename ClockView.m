  ...
  if ((self = [super init]) != nil)
    {
      ...
      NSMutableAttributedString *clockString = [[NSMutableAttributedString alloc] initWithString: @"Mon JAN XX XX:XX PM" ];
...
- (void) updateClock
{
  ...
  NSArray *monthAbbreviations = @[@"", @"Jan", @"Feb", @"Mar", @"Apr", @"May", @"Jun", @"Jul", @"Aug", @"Sep", @"Oct", @"Nov", @"Dec"];

  NSString *newMonth = monthAbbreviations[[date monthOfYear]];
 
  if (hour != newHour || minute != newMinute || dayOfWeek != newDayOfWeek)
    {
      BOOL useAmPmTime = true; //[[NSUserDefaults standardUserDefaults]
      ...
      if (useAmPmTime)
        {
          ...
          [clockButton setTitle: [NSString stringWithFormat:
					      _(@"%@ %@ %i  %d:%02d %@ "), [ShortNameOfDay(dayOfWeek) capitalizedString], newMonth, newDayOfMonth, h, minute, AMPMStringForHour(hour)]];
        }
      else
        {
          [clockButton setTitle: [NSString stringWithFormat: _(@"%@ %@ %i %d:%02d  "),
					   [ShortNameOfDay(dayOfWeek) capitalizedString], newMonth, newDayOfMonth, hour, minute]];
	}
    }
}
